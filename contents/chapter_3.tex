...
ToolBench的整体构造基于开放的API托管平台“RapidAPI Hub”，其API工具以层级化结构组织。从大类别（如天气、金融、翻译等）到工具组（tool groups），再到具体的API工具，层层递进。用户通过订阅工具集即可访问对应的API工具。数据集还包含多种类型的信息，例如工具集的详细描述（名称、描述、类别等）、工具输入输出参数列表、示例代码和工具调用路径数据。这些信息为研究工具调用任务提供了全面支持。在本研究中，重点使用了ToolBench中的工具调用路径数据，构建了工具图谱。这些路径数据记录了工具协作完成任务的调用顺序和逻辑关系，为工具图谱的构建奠定了数据上的基础。
...
\begin{table}[ht]
    \centering
    \bicaption{工具实体数据格式}{CSV Data Structure for Tool Entities}
...
\begin{figure}[H]
    ...
    \bicaption{基于大语言模型的工具关系抽取}{Relation Extraction with LLM}
...
\begin{table}[H]
    \centering
    \bicaption{工具图谱关系依赖抽取prompt构建示例}{Example Prompt for Extracting Resource Dependencies} % 表头
...
\begin{table}[H]
    \centering
    \bicaption{工具间强参数依赖关系数据格式}{CSV Data for Strong Parameter Dependencies}
    ...
    \begin{table}[H]
        \centering
        \bicaption{工具间弱参数依赖关系数据格式}{CSV Data for Weak Parameter Dependencies}
